...
if (not (args.seed or args.eini or args.total_distance or args.nevents)):
  raise RuntimeError('You must specify a seed (-s), an initial energy in GeV (-e), total propagation distance in km (-d) and number of events to simulate (-n)') 
...
class CasinoEvent(object):
  def __init__(self, particle_id, energy, position, TotalDistance, density):
  ...
  def SampleFinalTauParams(self):
    ...
    den = self.density*(units.cm**3)/units.gr   #convert density back to normal (not natural) units 
    ...
    cmd = 'awk "BEGIN {{for(i=0; i<1; i++) print 100000000, {ein} }}" | /data/user/isafa/MMC/MMC/ammc -run -frejus -tau -medi="Frejus rock" -radius=1e6 -vcut=1.e-3 -rho={rho} -scat -lpm -bs=1 -ph=3 -bb=2 -sh=1 -ebig=1e12 -seed=1223 -tdir=/data/user/isafa/MMC/tables/'.format(ein=e, rho=mult)
    #run MMC
    out = os.popen(cmd).read()
    #parse output
    index = out.find('\n')
    e_final = float(out[:index])                #MMC returns energy in GeV
...
def RollDice(initial_neutrino_energy, TotalDistance,
             density = 3.81*units.gr/(units.cm**3)):

    FirstEvent = CasinoEvent("tau_neutrino",initial_neutrino_energy,0.0, TotalDistance, density)
    EventCollection = [FirstEvent]

#    Rearth = 6371.
#    TotalDistance = 2.*Rearth*np.sin(zenith) 

#    if(np.sqrt(Rearth**2 - (TotalDistance**2/4.)) <= r):
    ...
    while(not np.any(map(lambda e: (e.position >= TotalDistance) or (e.energy <= e.GetMass()), EventCollection))):
        for event in EventCollection:
          ...
          elif(event.particle_id == 'tau_neutrino'):
...
for event in CasinoGame:
    if (event.particle_id == 'tau'):
        taus_e.append((event.energy, event.nCC, event.nNC, event.ntdecay))
    else:
        nus_e.append(event.energy)

np.save('./taus/taus_onTheOtherSide_DarkMatter_'+str(seed)+'_'+str(eini/units.GeV)+'GeV.npy', taus_e)
np.save('./nus/nus_onTheOtherSide_DarkMatter_'+str(seed)+'_'+str(eini/units.GeV)+'GeV.npy', nus_e)
